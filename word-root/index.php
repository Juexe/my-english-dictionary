...
if (!empty($_GET['text'])) {
    $text = trim($_GET['text']);
    if (!empty($text)) {
        $sql  = "SELECT f.word, LENGTH(f.word) AS word_len, f.translation AS trans, f.phonetic AS ph , f.collins AS col, f.w2 AS word2, f.t2 AS trans2, f.p2 AS ph2, f.c2 AS col2 FROM ( SELECT * FROM mydict froms JOIN ( SELECT word AS w2, translation AS t2, phonetic AS p2, collins AS c2 , RIGHT(word, LENGTH(word) - ?) AS word_r FROM mydict WHERE word LIKE ? ) tos WHERE BINARY froms.word = BINARY tos.word_r AND froms.phonetic IS NOT NULL ) f ORDER BY word_len DESC;";
        $find = "$text%";
...
<div id="content">
    <div id="input-area">
        ...
        <label>
            <form onsubmit="return search();">
                <input id="text" type="text" placeholder="前缀搜索：ac、dis..." value="<?= @$_GET['text'] ?>">
            </form>
        </label>
    ...
    <script>
        function search() {
            ...
            this.location.href = '/dict/word-root/?text=' + text2;
            return false;
        }
    </script>
    <?php if (!empty($res)): ?>
        <div id="word-list">
            <div class="word-item" style="font-weight: bold">
                <div class="item-left">
                    <span class="word-word">带该前缀 👇</span>
                </div>

                <div class="item-right">
                    <span class="word-word">去掉前缀 👇</span>
                </div>
            </div>
